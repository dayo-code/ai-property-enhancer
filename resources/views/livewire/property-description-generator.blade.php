<div class="max-w-7xl mx-auto p-4 sm:p-6 lg:p-8">
        ...
        <div class="lg:w-80 order-2 lg:order-1">
            <div class="card sticky top-6">
                <div class="flex items-center justify-between mb-4">
                    ...
                    @if($this->history->count() > 0)
                        <button
                            wire:click="clearAllHistory"
                            wire:confirm="Are you sure you want to delete all history?"
                ...
                @if($this->history->count() > 0)
                    <div class="space-y-3 max-h-[600px] overflow-y-auto">
                        @foreach($this->history as $item)
                            <div class="p-3 bg-gray-50 rounded-lg hover:bg-gray-100 transition border border-gray-200 @if($loadedHistoryId === $item->id) ring-2 ring-primary-500 @endif">
                                <div class="flex items-start justify-between gap-2 mb-2">
                                    <h4 class="font-medium text-sm text-gray-900 line-clamp-1">
                                        {{ $item->title }}
                                    </h4>
                                    <button
                                        wire:click="deleteFromHistory({{ $item->id }})"
                                ...
                                <div class="flex items-center justify-between">
                                    ...
                                    <button
                                        wire:click="loadFromHistory({{ $item->id }})"
    ...
    <div class="card mb-6">
        <form wire:submit="generateDescription" class="space-y-6">
            ...
            <div>
                ...
                <textarea
                    id="keyFeatures"
                    wire:model="keyFeatures"
                    rows="4"
                    class="input-field @error('keyFeatures') border-red-500 @enderror"
                    placeholder="e.g., Swimming pool, 24/7 security, Fully fitted kitchen, Ample parking space"
                ></textarea>
                @error('keyFeatures')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
                    ...
                    @foreach($this->tones as $value => $label)
                        <label class="relative flex items-center cursor-pointer">
                            <input
                                ...
                                class="peer sr-only"
                            >
                            <div class="w-full px-4 py-3 rounded-lg border-2 border-gray-300 peer-checked:border-primary-600 peer-checked:bg-primary-50 transition-all duration-200 hover:border-gray-400">
                                <div class="flex items-center gap-2">
                                    <div class="w-4 h-4 rounded-full border-2 border-gray-300 peer-checked:border-primary-600 peer-checked:bg-primary-600 flex items-center justify-center">
                                        <div class="w-2 h-2 rounded-full bg-white hidden peer-checked:block"></div>
                                    </div>
                                    <span class="text-sm font-medium text-gray-700 peer-checked:text-primary-700">
                                        {{ $label }}
                                    </span>
                                </div>
                            </div>
    ...
    @if($generatedDescription)
        <div
            class="card bg-gradient-to-br from-primary-50 to-blue-50 border border-primary-200"
            x-data="{ copied: false }"
        >
            <div class="flex items-center justify-between mb-4">
                ...
                @if($generationCount > 0)
                    <div class="mt-4 pt-4 border-t border-gray-200">
                        <p class="text-xs text-gray-500 flex items-center gap-1">
                            ...
                            Generated {{ $generationCount }} {{ $generationCount === 1 ? 'time' : 'times' }} • {{ ucfirst($tone) }} tone
                        </p>
                    </div>
                @endif
            </div>
            ...
            @if($scores)
                <div class="bg-white rounded-lg p-6 mb-4 shadow-sm">
                    ...
                    <!-- Overall Score -->
                    <div class="mb-4 p-4 bg-gradient-to-r from-{{ $this->scoringService->getScoreColor($scores['overall_score']) }}-50 to-{{ $this->scoringService->getScoreColor($scores['overall_score']) }}-100 rounded-lg border border-{{ $this->scoringService->getScoreColor($scores['overall_score']) }}-200">
                        <div class="flex items-center justify-between">
                            <span class="text-sm font-medium text-gray-700">Overall Score</span>
                            <span class="text-2xl font-bold text-{{ $this->scoringService->getScoreColor($scores['overall_score']) }}-700">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            <div class="flex items-center justify-between mb-2">
                                ...
                                <span class="text-sm font-semibold text-{{ $this->scoringService->getScoreColor($scores['readability_score']) }}-700">
                        ...
                        <div>
                            <div class="flex items-center justify-between mb-2">
                                ...
                                <span class="text-sm font-semibold text-{{ $this->scoringService->getScoreColor($scores['seo_score']) }}-700">
                    ...
                    <div class="mt-4 pt-4 border-t border-gray-200 grid grid-cols-2 md:grid-cols-4 gap-4">
                        ...
                        <div class="text-center">
                            <p class="text-2xl font-bold text-gray-900">{{ $scores['average_sentence_length'] }}</p>
